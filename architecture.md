...
The Academic Self-RAG Agent resolves these issues by embedding self-reflection loops directly into the state graph execution flow. At each stage, lightweight LLM evaluators grade document relevance (using efficient batched grading), hallucination/groundedness, and answer satisfaction before committing to a final output—falling back to live web search when local academic paper knowledge is insufficient. An automated benchmarking subsystem evaluates performance against Naive RAG baselines.

---

## 🏛️ High-Level System Architecture

The project consists of four core subsystems:
...
2. **LangGraph Self-RAG Engine**: Manages the state machine, document retrieval, batched relevance grading, generation, hallucination checking, answer evaluation, and conditional routing loops.
3. **Evaluation Subsystem**: Benchmarks Self-RAG against Naive RAG on a dataset of 26 questions, measuring Faithfulness, Relevancy, Web Fallback Accuracy, and Hallucination Interception.
4. **Streamlit UI & Control Dashboard**: Provides an interactive chat interface, execution logging, retrieved chunk inspection, and visual metric dashboards (document relevance breakdown, groundedness status, and web fallback previews).
...
graph TD
    ...
    subgraph Eval ["Evaluation Subsystem (src/agent/ & scripts/)"]
        EvalRunner["Benchmark Runner (scripts/run_eval.py)"]
        EvalEngine["Evaluator (src/agent/evaluator.py)"]
        NaiveRAG["Naive RAG Pipeline (src/agent/naive_rag.py)"]
        Dataset["Dataset (data/eval_dataset.json)"]
    end
    ...
    EvalRunner --> Dataset
    EvalRunner --> EvalEngine
    EvalEngine --> StateGraph
    EvalEngine --> NaiveRAG
```
...
flowchart TD
    ...
    retrieve --> grade_documents["2. Grade Documents\n(Batched 1-Call Flash LLM: relevant / not relevant)"]
...
| **`grade_documents`** | `nodes.grade_documents` | Filters retrieved chunks by evaluating relevance against user question using **batched single-call grading** (1 API call for all $K$ chunks). Attaches grade reasons. | `gemini-3.5-flash-lite` (JSON mode, `temp=0.0`) |
...
| **`generate`** | `nodes.generate` | Formulates structured prompt with graded document context or web fallback. Enforces inline paper citations. | `gemini-3.1-flash-lite` (`temp=0.2`) |
| **`grade_generation`** | `nodes.grade_generation` | Self-RAG hallucination check: verifies whether generated claims are strictly supported by context facts. | `gemini-3.5-flash-lite` (JSON mode, `temp=0.0`) |
| **`grade_answer`** | `nodes.grade_answer` | Self-RAG answer relevance check: evaluates if the output directly addresses the user's intent. | `gemini-3.5-flash-lite` (JSON mode, `temp=0.0`) |
...
* **Gemini API & Rate Limit Resilience ([`llm.py`](file:///home/shanujya/academic-rag-agent/src/agent/llm.py))**:
  * Intercepts `429 RESOURCE_EXHAUSTED` and transient `503/500` errors from the Gemini API.
  * Uses regex pattern matching on exception strings to parse server-suggested retry delays (e.g. `retry in 58.2s`), adding a safety margin before retrying automatically (up to 10 retry attempts).
  * Uses `gemini-3.5-flash-lite` for fast JSON grading tasks (`response_mime_type="application/json"`).
  * Uses `gemini-3.1-flash-lite` for qualitative answer synthesis.

---

### 3. Evaluation & Benchmarking Subsystem (`src/agent/` & `scripts/`)

```
src/agent/
├── evaluator.py     # Metric calculation (Faithfulness, Relevancy, Interception)
└── naive_rag.py     # Simple un-reflected RAG baseline implementation
scripts/
├── generate_eval_dataset.py # Automated synthetic benchmark question generator
└── run_eval.py              # CLI evaluation runner with side-by-side comparison
```

* **Metrics & LLM Judge ([`evaluator.py`](file:///home/shanujya/academic-rag-agent/src/agent/evaluator.py))**:
  * **Faithfulness**: LLM judge (`gemini-3.5-flash-lite`, `temp=0.0`) assesses whether generated claims are strictly grounded in retrieved context.
  * **Relevancy**: LLM judge evaluates if the answer directly addresses the prompt.
  * **Web Fallback Accuracy**: Compares actual web trigger usage against ground-truth labels.
  * **Hallucination Interception Rate**: Measures how effectively Self-RAG's internal reflection loop detects and corrects hallucinations during execution.
* **Naive RAG Baseline ([`naive_rag.py`](file:///home/shanujya/academic-rag-agent/src/agent/naive_rag.py))**:
  * Provides a direct comparative baseline by executing standard top-$K$ retrieval and single-pass answer generation without reflection or fallback logic.

---

### 4. User Interface Layer ([`app.py`](file:///home/shanujya/academic-rag-agent/app.py))
...
sequenceDiagram
    ...
    Note over Nodes,Gemini: Batched Single Call for all K chunks
    Nodes->>Gemini: grade_with_flash(batched prompt)
    Gemini-->>Nodes: [{"chunk_index": 1, "binary_score": "yes", ...}, ...]
...
| `FLASH_MODEL` | `"gemini-3.5-flash-lite"` | Fast LLM used for structured JSON reflection grading |
| `PRO_MODEL` | `"gemini-3.1-flash-lite"` | Main LLM used for synthesized academic answer generation |
| `JUDGE_MODEL` | `"gemini-3.5-flash-lite"` | LLM used for independent evaluation metric scoring |
...
2. **API Rate-Limit Backoff**:
   * Automatic exception parsing for HTTP 429 and 5xx status codes with up to 10 retry attempts prevents script failures during Gemini API usage.
3. **Batched Grading Efficiency**:
   * Single batched LLM grading reduces API request volume per question by up to 80%, conserving rate limits.
...
5. **Layout-Preserving Text Processing**:
